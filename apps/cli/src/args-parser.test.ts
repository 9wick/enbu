...
describe('parseArgs', () => {
  ...
  /**
   * A-17: --parallel <N>オプション
   *
   * 前提条件: argv = ['--parallel', '4']
   * 検証項目: { parallel: 4 } が返される
   */
  it('A-17: --parallelオプションをパースできる', () => {
    // Arrange
    const argv = ['--parallel', '4'];
  ...
  /**
   * A-18: --parallel 無効な値（文字列）
   *
   * 前提条件: argv = ['--parallel', 'invalid']
   * 検証項目: err({ type: 'invalid_args' }) が返される
   */
  it('A-18: --parallelに無効な値（文字列）が指定された場合、エラーを返す', () => {
    // Arrange
    const argv = ['--parallel', 'invalid'];
  ...
  /**
   * A-19: --parallel 無効な値（0以下）
   *
   * 前提条件: argv = ['--parallel', '0']
   * 検証項目: err({ type: 'invalid_args' }) が返される
   */
  it('A-19: --parallelに0以下の値が指定された場合、エラーを返す', () => {
    // Arrange
    const argv = ['--parallel', '0'];
   ...
   * 検証項目: err({ type: 'invalid_args' }) が返される
   */
  it('A-20: --parallelに値が指定されなかった場合、エラーを返す', () => {
    // Arrange
    const argv = ['--parallel'];
    ...
    result.match(
      ...
      (error) => {
        expect(error.type).toBe('invalid_args');
        expect(error.message).toContain('requires a number');
      },
    );
  });
});
