...
/** セレクタありの入力を作成 */
const withSelector = (selector: string): SelectorInput => ({ type: 'hasSelector', selector });

/** セレクタなしの入力 */
const noSelector: SelectorInput = { type: 'noSelector' };
...
describe('autoWait', () => {
   ...
   * 検証項目: ok({ type: 'resolved', resolvedRef: '@e1' }) が返される
   */
  it('AW-1: 要素が最初のsnapshotで見つかる場合、すぐに成功を返す', async () => {
    ...
    // Act: ログイン要素を待機
    const promise = autoWait(withSelector('ログイン'), mockContext);
    ...
    result.match(
      (value) => {
        expect(value.type).toBe('resolved');
        if (value.type === 'resolved') {
          expect(value.resolvedRef).toBe('@e1');
        }
      },
      () => {
  ...
  it('AW-3: タイムアウトまで要素が見つからない場合、timeoutエラーを返す', async () => {
    ...
    // Act: 存在しない要素を待機
    const promise = autoWait(withSelector('NotExist'), mockContext);
   ...
   *
   * 前提条件: noSelector 入力
   * 検証項目: ok({ type: 'skipped' }) が返される（待機スキップ）
   */
  it('AW-4: セレクタがない場合、スキップして成功を返す', async () => {
    // Act: noSelector入力で待機
    const result = await autoWait(noSelector, mockContext);

    // Assert: スキップして成功が返される
    expect(result.isOk()).toBe(true);
    result.match(
      (value) => expect(value.type).toBe('skipped'),
      () => {
        throw new Error('Expected ok result');
      },
    );
  ...
  it('AW-5: 参照ID形式のセレクタで要素が見つかる', async () => {
    ...
    // Act: @e1 参照IDで待機
    const promise = autoWait(withSelector('@e1'), mockContext);
    await vi.runAllTimersAsync();
    const result = await promise;
  ...
  it('AW-7: 複数の要素にマッチする場合、command_execution_failedを返す', async () => {
    ...
    // Act: 「詳細を見る」を待機
    const promise = autoWait(withSelector('詳細を見る'), mockContext);
    await vi.runAllTimersAsync();
    const result = await promise;
   ...
   * 検証項目: ok({ type: 'resolved', resolvedRef: '@e2' }) が返される
   */
  it('AW-8: 一意にマッチする場合は成功を返す', async () => {
    ...
    result.match(
      (value) => {
        expect(value.type).toBe('resolved');
        if (value.type === 'resolved') {
          expect(value.resolvedRef).toBe('@e2');
        }
      },
      () => {
        throw new Error('Expected ok result');
