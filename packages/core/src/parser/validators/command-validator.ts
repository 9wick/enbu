...
import { normalizers } from './normalizers/index.barrel';
...
export const validateCommand = (
  ...
  // 全てのnormalizerを試行してCommandを取得
  // normalizerはスキーマ経由でBranded Type化されたCommandを返す
  for (const normalizer of normalizers) {
    const result = normalizer(rawCommand);
    if (result !== null) {
      // スキーマの出力は既にBranded Type化されたCommand
      return ok(result);
    }
  }
